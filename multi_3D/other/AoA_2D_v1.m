...
%% Separate fast and slope ramp samples 
...
% end
FFT_ph_dir_slope = (FFT_ph_dir(2000)-FFT_ph_dir(1000))/1000;
mov_start_idx = round(1500-median(FFT_ph_dir(1001:2000))/FFT_ph_dir_slope);
N_frm_mov =  T_mov/Ts;
...
if IF.save_bg
    P_bg = P;
    save(['./matfile/P_bg_',date_ref],'P_bg');
end

%% background subtraction
if IF.bg_sub
    load(['./matfile/P_bg_',date_ref]);
...
%% Polar
    ...
    %     theta_plt = theta*2-90;
%     theta_range = find((theta_plt>=0)&(theta_plt<=180));
%     P_bg_sub_mid = P_bg_sub(:,theta_range); 
%     theta_plt_mid = theta_plt(theta_range);
%     P_mid = P(:,theta_range); 
%     P_bg_mid = P_bg(:,theta_range); 

    figure('units','normalized','outerposition',[0 0 1 1]);
    fig_polar_cu = polarPcolor(d_axis,theta_plt,P);
    camroll(90);
    ...
    figure();
    polarPcolor(d_axis,theta_plt,P_bg);
    camroll(90);
    title(['back ground polar ',file_ref],'Interpreter', 'none');    colormap jet
    caxis([0,50000000]); 
    
    figure('units','normalized','outerposition',[0 0 1 1]);
    fig_polar_bg = polarPcolor(d_axis,theta_plt,P_bg_sub);
    camroll(90);
    title(['background subtraction ',file_ref],'Interpreter', 'none');    colormap jet
    caxis([-25000000,25000000]);
    
    
    saveas(fig_polar_cu,sprintf('../figure/%s/o%s_r%d.fig',date_ref,obj_ref,run_ref));
    saveas(fig_polar_cu,sprintf('../figure/%s/o%s_r%d.jpg',date_ref,obj_ref,run_ref));
    saveas(fig_polar_bg,sprintf('../figure/%s/o%s_r%d_bg.fig',date_ref,obj_ref,run_ref));
    saveas(fig_polar_bg,sprintf('../figure/%s/o%s_r%d_bg.jpg',date_ref,obj_ref,run_ref));
